...
function get_exponential_parameters(N,M,σ)
    gx = rand(Uniform(0.75,1.1)) .+ rand(Uniform(-σ,σ), N)
    gs = rand(Uniform(1,2)) .+ rand(Uniform(-σ,σ), N)
    mx = rand(Uniform(0.9,1.25)) .+ rand(Uniform(-σ,σ), N)
...
function modular_community(N, M, f, C, T)
    c = x = MiCRM_stability.coherent_community(N,M, 3.0, 0.1, T)
...
Threads.@threads for pi = p_itt
    ...
    for (i,r) = pi
        k[2] += 1
        print("\r",k)
        p,U = modular_community(N, M, get_exponential_parameters, 0.3, T_vec[i])
        J = zeros(N+M, N+M)
